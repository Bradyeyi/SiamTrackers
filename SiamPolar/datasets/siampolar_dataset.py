...
class TrkDataset(Dataset):
    ...
    def shuffle(self):
        ...
        logger.info("datasets length {}".format(self.num))
    ...
    def get_mask(self, bbox):
        ...
        for i in range(0, 360, 10):
            x = center_x + a * math.cos(i)
            y = center_y + b * math.sin(i)
            mask.append([x, y])
    ...
    def __getitem__(self, index):
        ...
        # get mask image
        # if dataset.hashmask and not neg:
        #     mask_image = cv2.imread(search[2])
        #     countour = cv2.
        # else:
        ...
        cls = np.zeros((1, cfg.TRAIN.OUTPUT_SIZE, cfg.TRAIN.OUTPUT_SIZE), dtype=np.int64)

        template = template.transpose((2, 0, 1)).astype(np.float32)
        search = search.transpose((2, 0, 1)).astype(np.float32)

        # from bbox get mask 1* 4 =>> 36 * 2
        mask = self.get_mask(bbox)
        mask = np.array(mask)
        mass_center = np.mean(mask, axis=0)
